# The Hitchhikers Guide to `fe-marketplace`
...
- `internals/generators`: This folder has the code to scaffold out new components, containers and routes. Read [more about scaffolding](https://github.com/tophatmonocle/fe-marketplace/tree/master/docs/general#quick-scaffolding) in the docs.
...
Like any other webpage your app starts with the [`app/index.html`](https://github.com/tophatmonocle/fe-marketplace/blob/master/app/index.html) file. React will render your application into the `div#app` .
...
Webpack requires an entry point to your application. Think of it as a door to your source code. In this project [`app/app.js`](https://github.com/tophatmonocle/fe-marketplace/blob/master/app/app.js) is that entry point. Webpack will access the entire app from this file, transpile the application into ES5 and create small chunks of transpiled code. Only the required chunks will be loaded in the browser so that you don't have to worry about the size of your application.
...
- `ReactDOM.render()` not only renders the [root react component](https://github.com/tophatmonocle/fe-marketplace/blob/master/app/containers/App/index.js) called `<App />`, of your application, but it renders it with `<Provider />`, `<LanguageProvider />` and `<Router />`.
...
The Redux `store` is the heart of your application. Check out [`configureStore.js`](https://github.com/tophatmonocle/fe-marketplace/blob/master/app/configureStore.js) to see how we have configured the store.
...
The fe-marketplace building blocks interoperate to produce a seamless application. Let's join these pieces together.

<img src="workflow.png" alt="fe-marketplace workflow" align="center" />
...
Run `npm start` to launch the application. If you start browsing at [https://localhost:3000](https://localhost:3000), by default you will be navigated to the home page. Here, notice that route is `"/"`, so the [`<HomePage />`](https://github.com/tophatmonocle/fe-marketplace/blob/master/app/containers/HomePage/index.js) container will be mounted. It is responsible for rendering a form with a textbox and a list of repositories.
...
Check out [`HomePage/saga.js`](https://github.com/tophatmonocle/fe-marketplace/blob/master/app/containers/HomePage/saga.js). It can be confusing for untrained eyes. The API of `redux-saga` is self-descriptive once you've seen it, so let's go over what happens in there:
...
- If the API successfully returns some data, a `reposLoaded()` action will be dispatched which carries the data. When redux store receives this action, [a reducer](https://github.com/tophatmonocle/fe-marketplace/blob/master/app/containers/App/reducer.js) will set incoming data in the new state tree.
...
In the example app, we're basically using a [bazooka](https://en.wikipedia.org/wiki/Bazooka) to kill a fly. The primary purpose of this project is to provide a robust and scalable infrastructure for large scale projects. Every bit of your project should be predictable. You should have granular control over all the operations. It's overkill for the example app, but not for the real world.

If you want to build a mature application you're going to need a mature infrastructure. That's what this project provides.
